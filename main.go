...
var fileStructure = map[string]string{
	...
	"/internal/assert/assert.go":    "templates/assert.txt",
	"/internal/assert/testdata.go":  "templates/testdata.txt",
	"/internal/assert/testutils.go": "templates/testutils.txt",
...
func main() {
	...
	for targetPath, templatePath := range fileStructure {
		...
		updatedContent = strings.ReplaceAll(updatedContent, "{{RepoPath}}", "github.com/yourusername/"+*repoName)
